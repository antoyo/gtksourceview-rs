...
pub trait BufferExt {
    ...
    fn connect_bracket_matched<F: Fn(&Self, &gtk::TextIter, BracketMatchType) + 'static>(&self, f: F) -> u64;
...
impl<O: IsA<Buffer> + IsA<glib::object::Object>> BufferExt for O {
    ...
    fn connect_bracket_matched<F: Fn(&Self, &gtk::TextIter, BracketMatchType) + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Self, &gtk::TextIter, BracketMatchType) + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "bracket-matched",
                transmute(bracket_matched_trampoline::<Self> as usize), Box_::into_raw(f) as *mut _)
        }
    }
...
unsafe extern "C" fn bracket_matched_trampoline<P>(this: *mut ffi::GtkSourceBuffer, iter: *mut gtk_ffi::GtkTextIter, state: ffi::GtkSourceBracketMatchType, f: glib_ffi::gpointer)
where P: IsA<Buffer> {
    callback_guard!();
    let f: &Box_<Fn(&P, &gtk::TextIter, BracketMatchType) + 'static> = transmute(f);
    f(&Buffer::from_glib_none(this).downcast_unchecked(), &from_glib_none(iter), from_glib(state))
}
